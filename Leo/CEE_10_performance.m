%% 10. Performance evaluation including nonlinearities
%% Init
clear;
...
for ictrl = 1:length(Qctrl)
    for iobs = 1:length(Qobs)
        ...
        Q_ = [Q zeros(5,1); zeros(1,5) Qctrl(ictrl)*1/(10*0.18)^2];
        [K,~,~] = lqr(A_,B_,Q_,R,0);
        ABK_values = eig(A_ - B_*K);
...
%% Save data
str = sprintf("perfromance_Int_%02d.mat",RUN);
...
str = sprintf("perfromance_Int_Min%02d.mat",RUN);
